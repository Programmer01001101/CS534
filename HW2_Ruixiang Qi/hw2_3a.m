...
syms x y
f = @(x,y) -0.5*log(det(s1)) - 0.5* ([x; y]-m1)'*inv(s1)*([x ; y]-m1) + log(pi) - (-0.5*log((det(s2))) - 0.5* ([x; y]-m2)'*inv(s2)*([x;y]-m2) + log(pi));
...
%title, legend, axes
title('3.a. Quadratic discriminant')
p1 = plot(1,1,'g');
p2 = plot(2,2,'k');
p3 = plot(3,3,'--k');
legend([p1 p2 p3],{'QDA','Bayes(theoretical)','Bayes(estimated)'})
xlabel('x1') 
ylabel('x2') 
axis([-2 7 -3 5])
